...
generator_model_paths = [f"gpt2_round{i}" for i in [1, 3, 5, 7]]
num_samples = 200  # per model
...
star_counts = {star: int(num_samples * ratio) for star, ratio in star_ratios.items()}

# Ensure output dir exists
os.makedirs(output_dir, exist_ok=True)

# Generation function
def generate_review(star, categories, model, tokenizer, device, temperature=0.8):
...
# Loop over G models
for model_path in generator_model_paths:
    ...
    for star, count in star_counts.items():
        for _ in tqdm(range(count), desc=f"Stars: {star}"):
            ...
            review = generate_review(star, cat, model, tokenizer, device)
    ...
    # Save result
    output_file = os.path.join(output_dir, f"{model_path}_samples.csv")
